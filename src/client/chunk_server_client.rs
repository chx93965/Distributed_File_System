...
impl ChunkServerClient {
    pub fn new(base_url: &str) -> Self {
        ChunkServerClient {
            chunkserver_url: base_url.to_string(),
            client: Client::new(),
        }
    }

    /// Fetch hello message from the server.
    pub async fn contact(&self) -> Result<String, Box<dyn Error>> {
        let url = format!("{}/", self.chunkserver_url);
    ...
    pub async fn add_chunk(&self, id: &str, data: &[u8]) -> Result<(), Box<dyn Error>> {
        let url = format!("{}/add_chunk", self.chunkserver_url);
        let response = self
            .client
            .post(&url)
            .query(&[("id", id)])
    ...
    pub async fn append_chunk(&self, id: &str, data: &[u8]) -> Result<(), Box<dyn Error>> {
        let url = format!("{}/append_chunk", self.chunkserver_url);
        let response = self
            .client
            .post(&url)
            .query(&[("id", id)])
    ...
    pub async fn get_chunk(&self, id: &str) -> Result<Vec<u8>, Box<dyn Error>> {
        let url = format!("{}/get_chunk", self.chunkserver_url);
        let response = self
            .client
            .get(&url)
            .query(&[("id", id)])
    ...
    pub async fn get_chunk_list(&self) -> Result<Vec<String>, Box<dyn Error>> {
        let url = format!("{}/get_chunk_list", self.chunkserver_url);
        let response = self.client.get(&url).send().await?;
